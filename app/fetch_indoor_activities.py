...
# Categories for indoor activities
CATEGORIES = 'museums,galleries,bowling,escapegames,movietheaters,arcades,indoor_playcenter,gyms,yoga,billiards,dancestudio,trampoline,rock_climbing,cookingclasses,libraries,musicvenues,theater,karaoke,science_centers,education'  
LIMIT = 50  # Maximum number of results per request
TOTAL_RESULTS = 500  # Total number of results desired

def fetch_indoor_activities():
    all_activities = []
...
def save_to_csv(activities, filename='indoor_activities.csv'):
...
if __name__ == '__main__':
    activities = fetch_indoor_activities()
    save_to_csv(activities)
    print(f"Saved {len(activities)} indoor activities to 'indoor_activities.csv'")
